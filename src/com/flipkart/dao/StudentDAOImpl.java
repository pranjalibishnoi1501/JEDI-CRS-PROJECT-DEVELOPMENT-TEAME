...
//			logger.error(e.getMessage());
	...
	public List<String> viewNotification(int userID){
		...
		try {
			PreparedStatement statement = connection.prepareStatement("Select * from Notification where userID=?");
			statement.setInt(1, userID);
			ResultSet rs=statement.executeQuery();
			...
			while(rs.next())
			{
				...
				noti.add(rs.getString(3));
			}
			System.out.println(noti.size());
			int rowsInserted = statement.executeUpdate();
			if (rowsInserted > 0) {
				System.out.println("Notification fetched successfully");
			} else {
				System.out.println("Failed to fetch notification.");
			}
		}
		catch(SQLException e)
		{
